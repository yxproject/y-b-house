window.OrgChart = function(t, e) {
    ...
    })), this.config.yScrollUI || (this.yScrollUI = new OrgChart.yScrollUI(this.element, this.config,
    function() {
        return {
            boundary: r.response.boundary,
            scale: r.getScale(),
...
OrgChart.prototype._draw = function(t, v, y, x) {
    ...
    this.server.read(t, this.width(), this.height(), e, v, y,
    function(t) {
        ...
        if (OrgChart.events.publish("render", [b, e]), r = e.content, r += b.ui.lonely(b.config), v !== OrgChart.action.exporting) {
            ...
            if (0 < p[0].length) {
                ...
                OrgChart.anim(p[0], p[1], p[2], b.config.anim.duration, b.config.anim.func,
                function() {
            ...
            b.config.anim.duration, b.config.anim.func,
            function() {
            ...
            500, OrgChart.anim.inOutPow,
            function() {
                b.xScrollUI.setPosition(),
                b.yScrollUI.setPosition(),
                g || (x && x(), OrgChart.events.publish("redraw", [b]), g = !0)
            }),
            b._initialized = !0
        } else {
            var u = t.boundary,
...
OrgChart.prototype.linkNode = function(t, e) {
    if (this._canUpdateLink(t, e)) {
        ...
        i = this.getBGNode(e);
        if (r.isGroup && i.isGroup) for (var a = i.bgnodes[0].id, o = 0; o < r.bgnodes.length; o++) { (n = this.get(r.bgnodes[o].id)).pid = a,
            this.update(n, !0)
        } else if (!r.isGroup && i.isGroup) {
            a = i.bgnodes[0].id; (n = this.get(t)).pid = a,
            this.update(n, !0)
        } else if (r.isGroup && !i.isGroup) for (o = 0; o < r.bgnodes.length; o++) { (n = this.get(r.bgnodes[o].id)).pid = e,
            this.update(n, !0)
        } else if (!r.isGroup && !i.isGroup) {
            var n; (n = this.get(t)).pid = e,
            this.update(n, !0)
        }
...
OrgChart.prototype.removeNode = function(t, e) {
    var r = this;
    return ! 1 !== this.remove(t, e) && (this._draw(!1, OrgChart.action.update, null,
    function() {
...
OrgChart.prototype.remove = function(t, e) {
    ...
    for (var i = null,
    a = this.config.nodes.length - 1; 0 <= a; a--) if (this.config.nodes[a].id == t) {
        i = this.config.nodes[a].pid,
        this.config.nodes.splice(a, 1);
        break
    }
    var o = this.getByParentId(t);
    for (a = o.length - 1; 0 <= a; a--) o[a].pid = i,
    this.update(o[a], e);
    return ! 0
},
...
OrgChart.prototype.group = function(t, e, r) {
    ...
    a = this.getBGNode(e),
    o = a.id;
    if (i.isGroup || a.isGroup || i.isChildOfGroup || a.isChildOfGroup) if (i.isGroup || a.isGroup || !i.isChildOfGroup || a.isChildOfGroup) if (i.isGroup || a.isGroup || i.isChildOfGroup || !a.isChildOfGroup) if (!i.isGroup && !a.isGroup && i.isChildOfGroup && a.isChildOfGroup) this._removeGroupIfHasOneNode(i._groupParentNodeId, i.id),
    this.removeNodeTag(i.id, i._groupParentNodeId, !0),
    this.addNodeTag(i.id, a._groupParentNodeId, !0),
    this.link(i.id, a.pid);
    else if (!i.isGroup && a.isGroup && i.isChildOfGroup && !a.isChildOfGroup) this._removeGroupIfHasOneNode(i._groupParentNodeId, i.id),
    this.removeNodeTag(i.id, i._groupParentNodeId, !0),
    this.addNodeTag(i.id, a.id, !0);
    else if (i.isGroup || !a.isGroup || i.isChildOfGroup || a.isChildOfGroup) {
        if (i.isGroup && a.isGroup && !i.isChildOfGroup && !a.isChildOfGroup) for (var n = 0; n < i.bgnodes.length; n++) this.removeNodeTag(i.bgnodes[n].id, i.id, !0),
        this.addNodeTag(i.bgnodes[n].id, a.id, !0)
    } else this.addNodeTag(i.id, a.id, !0);
    else this.addNodeTag(i.id, a._groupParentNodeId, !0),
    this.link(i.id, a.pid);
    else {
...
OrgChart.prototype._addEvent = function(e, t, r, i) {
    var a, o; (i || (i = ""), e.getListenerList || (e.getListenerList = {}), e.getListenerList[t + i]) || (a = this, o = r, r = function() {
    ...
    function() {
        ...
        return ! 1 === t && (window.event.returnValue = !1, window.event.cancelBubble = !0),
        t
...
OrgChart.input = function(t, e, r, i, a) {
    ...
    if (h.innerHTML = "Upload", h.style.position = "absolute", h.style.right = 0, o.style.margin = "14px 14px 7px 14px", o.style.textAlign = "left", o.style.position = "relative", o.setAttribute("data-field-name", t), s.style.border = "1px solid #d7d7d7", s.style.backgroundColor = "#d7d7d7", s.style.display = "block", s.style.width = "100%", n.style.color = "#bcbcbc", l.style.border = "none", l.style.outline = "none", l.style.width = "100%", l.setAttribute("val", ""), a && (l.style.width = "80%"), l.style.fontSize = "16px", l.readOnly = i, null != e && null != e && (l.value = e), null != r && null != r && (l.placeholder = r), null != t && null != t && (n.innerHTML = t), n.setAttribute("lbl", ""), l.style.color = "#7a7a7a", i || l.addEventListener("focus",
    function() {
...
OrgChart.editUI.prototype.hide = function(t) {
    if (this.wrapperElement) {
        ...
        } else this.obj.element.removeChild(this.wrapperElement),
...
OrgChart.menuUI.prototype._show = function(t, e, o, n, l) {
    ...
    if ("" != r) {
        ...
        for (var f = this.wrapper.getElementsByTagName("div"), u = 0; u < f.length; u++) { (a = f[u]).addEventListener("mouseover",
            function() {
...
OrgChart.idb._open = function(e) {
    if ((0 < navigator.userAgent.toLowerCase().indexOf("safari") || 0 < navigator.userAgent.toLowerCase().indexOf("firefox")) && window.location !== window.parent.location) e && e(!1);
        ...
        if (!window.indexedDB) return console.error("Your browser doesn't support a stable version of IndexedDB."),
        void(e && e(!1));
...
OrgChart.toolbarUI.prototype.init = function(t, e) {
    if (e) {
        ...
        this.layoutBtn && this.layoutBtn.addEventListener("click",
        function() {
            n._visible ? n.hideLayout() : n.showLayout()
        }),
        this.layouts && this.layouts.addEventListener("click",
        function(t) {
...
OrgChart.prototype._export = function(a, o) {
    ...
    this._draw(!1, OrgChart.action.exporting, t,
    function(t) {
        var r = document.createElement("div");
        if (r.innerHTML = t, "svg" == a.ext) if (o) o(a, r.innerHTML);
        else {
            var e = {
                content: r.innerHTML,
                options: a
        ...
        } else OrgChart._imgs2base64(r, "image", "xlink:href",
        function() {
            n._pages(a, r.querySelector("svg"),
            function(t) {
                var e = {
                    content: r.innerHTML,
                    options: a,
                ...
                o ? o(n, e, r.querySelector("svg")) : (e = JSON.stringify(e), OrgChart._ajax(n.config.exportUrl + "/v3", "POST", e, "arraybuffer",
                function(t) {
...
OrgChart.prototype._pagesA2Levels = function(n, l) {
    ...
    for (var r = 0; r < g.length; r++) {
        ...
        function(r) {
            s._draw(!1, OrgChart.action.exporting, t,
            function(t) {
                var a, o, e = document.createElement("div");
                e.innerHTML = t,
                a = e,
                o = r,
                OrgChart._imgs2base64(a, "image", "xlink:href",
                function() {
...
OrgChart.prototype.importCSV = function() {
    ...
    t.onchange = function(t) {
        ...
        s.onload = function() {
            for (var t = s.result,
            e = OrgChart._csvToArray(t, ","), r = [], i = 1; i < e.length; i++) {
                for (var a = {},
                o = 0; o < e[i].length; o++) {
                    var n = e[0][o],
                    l = e[i][o];
...
OrgChart.prototype._globalMouseDownHandler = function(t, e) {
    ...
    }), t == e.target) return e.stopPropagation(),
    e.preventDefault(),
    void this._mouseDownHandler(t, e, r);
    for (var i = e.target; i != t;) {
        if (i.hasAttribute("node-id")) return e.stopPropagation(),
        e.preventDefault(),
...
OrgChart.prototype._nodeMouseDownHandler = function(t, e, n) {
    if (this.config.enableDragDrop) {
        ...
        if (!1 !== OrgChart.events.publish("drag", [this, i])) {
            ...
            y = function(t) {
                if (c.removeEventListener(n.move, v), c.removeEventListener(n.up, y), n.leave && c.removeEventListener(n.leave, y), l.id == s || l.isChildOfGroup && l._groupParentNodeId == s || null == s) return O(s, h),
                c.removeChild(m),
                void(d._gragStartedId = null);
                var e = d.getBGNode(s);
                if (e.isChildOfGroup && l.isChildOfGroup && e._groupParentNodeId == l._groupParentNodeId) return O(s, h),
                c.removeChild(m),
                void(d._gragStartedId = null);
                if (!1 === OrgChart.events.publish("drop", [d, l.id, e.id])) return O(s, h),
                c.removeChild(m),
                void(d._gragStartedId = null);
...
OrgChart.prototype._lonelyButtonHandler = function() {
    var t = new OrgChart.node(OrgChart._randomId(), null, null, [], this.config.template); ! 1 !== this.addNode(t, !0) && this.center(t.id)
},
...
OrgChart.node = function(t, e, r, i, a) {
    ...
    this.w = o && o.size ? o.size[0] : 0,
    this.h = o && o.size ? o.size[1] : 0,
...
OrgChart.prototype._mouseDownHandler = function(t, e, r) {
    ...
    f = function() {
        "pan" == c.type && s.config.sticky ? OrgChart._moveToBoundaryArea(t, s.getViewBox(), s.response.boundary,
        function() {
...
OrgChart.remote._walk = function(r, t, i) {
    for (var e = [], a = [], o = [t.orientation, t.levelSeparation, t.mixedHierarchyNodesSeparation, t.subtreeSeparation, t.siblingSeparation, t.template], n = 0; n < r.length; n++) a.push(r[n].id),
    ...
    null != OrgChart.remote._fromReqDTO ? OrgChart.remote._fromReqDTO(l.n, l.r, l.c,
    function(t) {
        for (var e = 0; e < r.length; e++) OrgChart.remote._fromResDTO(r[e], t, 0, r);
        i()
    }) : (l = JSON.stringify(l), OrgChart.idb.read(l,
    function(t, e) {
        t ? OrgChart.remote._proceed(r, e.res, i) : OrgChart.remote._findRegion(function(t) {
            OrgChart._ajax(t, "post", l, "json",
            function(t) {
...
OrgChart.searchUI.prototype._serverSearch = function(t) {
    for (var e = this,
    r = this.obj.element.querySelector('[data-id="container"]'), i = this.obj.element.querySelector('[data-id="search"]'), a = this.obj.server.find(t), o = "", n = 0; n < a.length; n++) {
        var l = a[n],
        s = "";
    ...
    for (n = 0; n < h.length; n++) h[n].addEventListener("click",
    function() {
    ...
    h[n].addEventListener("mouseover",
    function() {
        this.setAttribute("data-selected", "yes"),
        this.style.backgroundColor = "#F0F0F0"
    }),
    h[n].addEventListener("mouseleave",
    function() {
...
OrgChart.server.prototype.read = function(t, r, i, e, a, o, n, l) {
    ...
    if (t) {
        ...
        a != OrgChart.action.exporting && (this.maxX = h, this.maxY = d, this.minX = c, this.minY = g, this.root = p, this.nodes = f, this.groupChildren = u, this.visibleNodes = C.visibleNodes),
        n(C)
    } else this.oldNodes = f || null,
    this._read(function(t) {
        ...
        e.notif = t.limit,
        a != OrgChart.action.exporting && (s.maxX = h, s.maxY = d, s.minX = c, s.minY = g, s.root = p, s.nodes = f, s.groupChildren = u, s.visibleNodes = e.visibleNodes),
        n(e)
    },
    l)
},
...
OrgChart.server.prototype.find = function(t) {
    ...
    for (s = 0; s < n.length && 10 != s; s++) {
        for (var g = !1,
        p = 0; p < l.length; p++) if (l[p].id == n[s].id) {
            g = !0;
            break
        }
        if (!g) {
            ...
            for (p = 0; p < O.length; p++) {
                d = O[p],
                c = n[s].data[d];
                d === n[s].propertyName && (e = c, i = t, void 0, a = OrgChart.accentFold(e).indexOf(i), c = e.toString().splice(a + i.length, 0, "</mark>").splice(a, 0, "<mark>")),
            ...
            l.push({
                id: n[s].id,
                node: new OrgChart.node(n[s], null, null, [], v),
                textId: u,
                textInNode: f,
                text: C,
                img: m
...
OrgChart.server._createNodes = function(t, e, r, i) {
    ...
    d = !1, c = 0; c < a.length; c++) {
        ...
        if ((y = new OrgChart.node(g.id, g.pid, C, p, u)).isAssistant = -1 != p.indexOf("assistant"), y.isAssistant && -1 == s.indexOf(y.pid) && s.push(y.pid), null != f) {
            ...
            o[f.name].bgnodes.push(y),
            1 == y.isAssistant && (o[f.name].isAssistant = !0),
            h[y.id] = f.name,
            d = !0
        } else null != t.orderBy && (y.order = g[t.orderBy]),
        o[g.id] = y,
        n.push(g.id)
    }
    if (null != t.orderBy && n.sort(function(t, e) {
        ...
        h[(y = o[n[c]]).pid] && y.id != h[y.pid] && (y.pid = h[y.pid])
    }
    var O = [];
    for (c = 0; c < n.length; c++) {
        ...
        y = o[v],
        x = i ? i[v] : null;
        if (null != y.pid && null != y.pid && "" !== y.pid)(_ = o[y.pid]) ? ((y.parent = _).children.push(y), _.childrenIds.push(y.id)) : (y.pid = null, y.parent = null, l.push(y));
        else l.push(y);
        e == OrgChart.action.exporting && null != r.id && (l = [o[r.id]]),
        x ? (y.state = x.state, y.state == OrgChart.COLLAPSE && O.push(y.id), OrgChart.server._setNodeGroupState(y, x.groupState, t.orientation)) : OrgChart.server._setNodeGroupState(y, OrgChart.EXPAND, t.orientation),
        OrgChart.server._initStates(y, o, e, t, r, O),
        OrgChart.server._initGroupStates(y, o, e, t, r)
    }
    if (null != t.roots) {
        l = [];
        for (c = 0; c < t.roots.length; c++) {
            var b = o[t.roots[c]];
            if (b) {
                if (b.parent) for (var w = (_ = b.parent).children.length - 1; 0 <= w; w--) { (z = _.children[w]) == b && (z.parent = null, _.children.splice(w, 1), _.childrenIds.splice(w, 1))
                }
                l.push(b)
            }
        }
    ...
    for (c = 0; c < s.length; c++) {
        var _ = o[s[c]],
        k = new OrgChart.node(_.id + "_split_0", _.id, null, [], "split"); (o[k.id] = k).isAssistant = !0,
        (x = i[k.id]) && (o[k.id].state = x.state, o[k.id].state == OrgChart.COLLAPSE && O.push(k.id)),
        OrgChart.server._initStates(k, o, e, t, r, O);
        var A = [];
        for (w = _.children.length - 1; 0 <= w; w--) { (z = _.children[w]).isAssistant ? (z.parent = null, _.children.splice(w, 1), _.childrenIds.splice(w, 1), A.splice(0, 0, z.id)) : (z.pid = k.id, (z.parent = k).children.push(z), k.childrenIds.push(z.id), _.children.splice(w, 1))
        }
        if (A.length % 2) {
            var S = o[A[A.length - 1]],
            I = [S.w, S.h],
        ...
        for (w = A.length - 1; 0 <= w; w--) {
            if (w % 2 && w != A.length - 1)(J = new OrgChart.node(_.id + "_split_" + M, null, null, [], "split")).isAssistant = !0,
            o[J.id] = J,
            A.splice(w, 0, J.id),
            M++;
        ...
        for (w = 0; w < A.length; w += 3) {
            ...
            for (var P = E;
            "split" == P.templateName && (P = P.parent);) P.childrenIds.push(L.id),
    ...
    if (e == OrgChart.action.centerNode) {
        var H = o[r.id];
        H || (H = o[h[r.id]], OrgChart.server._setNodeGroupState(H, OrgChart.EXPAND, t.orientation)),
        H.state = OrgChart.EXPAND,
        -1 != (D = O.indexOf(H.id)) && O.splice(D, 1);
        _ = OrgChart.server._getRealParent(H);
        for (var U = H.id; null != _;) {
            if ( - 1 != (D = O.indexOf(_.id)) && O.splice(D, 1), _.state = OrgChart.EXPAND, r.options.parentState === OrgChart.COLLAPSE_PARENT_NEIGHBORS) for (c = 0; c < _.childrenIds.length; c++) {
                var z = o[_.childrenIds[c]];
                if (!OrgChart.server._isSplit(z) && !z.isAssistant && z.id != U) - 1 == (D = O.indexOf(_.childrenIds[c])) && O.push(_.childrenIds[c]),
                z.state = OrgChart.COLLAPSE
            }
            if (_.state == OrgChart.EXPAND) for (var F = 0; F < _.childrenIds.length; F++) {
                var j = o[_.childrenIds[F]];
                if (OrgChart.server._isSplit(j) || j.isAssistant) - 1 != (D = O.indexOf(j.id)) && O.splice(D, 1),
                j.state = OrgChart.EXPAND
            }
            U = _.id,
            _ = OrgChart.server._getRealParent(_)
        }
        if (r.options.childrenState === OrgChart.COLLAPSE_SUB_CHILDRENS) for (c = 0; c < H.childrenIds.length; c++) {
            if ( - 1 != (D = O.indexOf(H.childrenIds[c])) && O.splice(D, 1), (z = o[H.childrenIds[c]]).state = OrgChart.EXPAND, !z.isAssistant && !OrgChart.server._isSplit(z)) for (w = 0; w < z.childrenIds.length; w++) {
                var D, G = o[z.childrenIds[w]]; - 1 == (D = O.indexOf(z.childrenIds[w])) && O.push(z.childrenIds[w]),
    ...
    for (c = 0; c < O.length; c++) {
        if (_ = (y = o[O[c]]).parent) for (w = _.children.length - 1; 0 <= w; w--) {
            if (_.children[w] == y) if ( - 1 == _.collapsedChildrenIds.indexOf(y.id) && _.collapsedChildrenIds.push(y.id), _.children.splice(w, 1), "split" == y.templateName || "split" == _.templateName) {
                if (E = y.parent) {
                    ...
                    for (var X = 0; X < E.childrenIds.length; X++) {
                        var R = E.childrenIds[X]; - 1 == E.collapsedChildrenIds.indexOf(R) && E.collapsedChildrenIds.push(R)
                    }
                }
            } else y.parent = null
        }
    }
    if (t.layout == OrgChart.mixed) for (c = 0; c < n.length; c++) {
        if (0 == (y = o[n[c]]).childrenIds.length && !y.isAssistant && "split" != y.templateName) if (_ = y.parent) {
            if (!OrgChart.server._mixedLayoutAllNodes(_)) continue;
            for (w = _.children.length - 1; 0 <= w; w--) {
                if ((z = _.children[w]) != y && 0 == z.childrenIds.length && !z.isAssistant && "split" != z.templateName) {
                    z.parent = y,
                    z.pid = y.id,
    ...
    if (t.layout == OrgChart.tree || t.layout == OrgChart.treeRightOffset || t.layout == OrgChart.treeLeftOffset) {
        var q = [];
        for (c = 0; c < n.length; c++) {
            if (0 == (y = o[n[c]]).childrenIds.length && !y.isAssistant && "split" != y.templateName)(_ = y.parent) && -1 == q.indexOf(_.id) && q.push(_.id)
        }
        for (c = 0; c < q.length; c++) {
            _ = o[q[c]],
            k = new OrgChart.node(_.id + "_split_0", _.id, null, [], "split"); (o[k.id] = k).layout = OrgChart.tree;
            var V = [];
            for (w = _.children.length - 1; 0 <= w; w--) {
                if (OrgChart.server._mixedLayoutAllNodes(_)) if (0 == (z = _.children[w]).childrenIds.length && !z.isAssistant && "split" != z.templateName && (z.parent && "split" != z.parent.templateName || z.parent.isAssistant)) {
                    if ((z = _.children[w]).parent = null, z.layout = OrgChart.tree, _.children.splice(w, 1), _.childrenIds.splice(w, 1), t.layout == OrgChart.treeRightOffset && V.splice(0, 0, z.id), t.layout == OrgChart.treeLeftOffset || t.layout == OrgChart.treeRightOffset) {
                        var W;
                        ...
                        (W = new OrgChart.node(z.id + "_transperant", null, null, [], N)).layout = OrgChart.tree,
                        o[W.id] = W,
                        V.splice(0, 0, W.id)
                    }
            ...
            for (M = 1, w = V.length - 1; 0 <= w; w--) {
                var J;
                if (w % 2 && w != V.length - 1)(J = new OrgChart.node(_.id + "_split_" + M, null, null, [], "split")).layout = OrgChart.tree,
                o[J.id] = J,
                V.splice(w, 0, J.id),
                M++;
            ...
            for (w = 0; w < V.length; w += 3) {
                ...
                B && (0 != w && (E = o[V[w - 2]]), B.parent = E, B.pid = E.id, E.children.push(B), _.childrenIds.push(B.id)),
                T && (0 != w && (E = o[V[w - 1]]), T.parent = E, T.pid = E.id, E.children.push(T), _.childrenIds.push(T.id))
            }
        }
    }
    return {
...
OrgChart.server._initStates = function(t, e, r, i, a, o) {
    ...
    } else if (r == OrgChart.action.expandCollapse && "expandCollapseToLevel" == a.method) { - 1 != (s = o.indexOf(t.id)) && o.splice(s, 1),
...
OrgChart.server._expandCollapseToLevel = function(t, e, r, i, a, o) {
    if (i.collapse.level) {
        ...
        if (n >= i.collapse.level) {
            ...
            } else s = -1 != i.expand.nodes.indexOf(t.id); ! s && i.collapse.allChildren ? (o.push(t.id), t.state = OrgChart.COLLAPSE) : s || i.collapse.allChildren || n != i.collapse.level || (o.push(t.id), t.state = OrgChart.COLLAPSE)
        }
    }
},
...
OrgChart.server._initNodeSize = function(t, e, r) {
    ...
    if (t.isGroup && t.groupState == OrgChart.EXPAND) {
        ...
        case OrgChart.orientation.bottom_left:
            l = 1,
            s = n;
            for (var c = 0; c < t.bgnodes.length; c++) h = Math.max(t.bgnodes[c].w, h),
        ...
        case OrgChart.orientation.left_top:
            l = n,
            s = 1;
            for (c = 0; c < t.bgnodes.length; c++) h = Math.max(t.bgnodes[c].w, h),
            d = Math.max(t.bgnodes[c].h, d),
...
OrgChart.server._setGroupNodes = function(t, e) {
    if (t.isGroup && t.groupState == OrgChart.EXPAND) {
        ...
        case OrgChart.orientation.bottom_left:
            for (var o = 0,
            n = 0; n < t.bgnodes.length; n++) { (s = t.bgnodes[n]).x = t._groupSettings._cellWidth / 2 - s.w / 2,
        ...
        case OrgChart.orientation.left_top:
            var l = 0;
            for (n = 0; n < t.bgnodes.length; n++) { (s = t.bgnodes[n]).y = t._groupSettings._cellHeight / 2 - s.h / 2,
                s.x = t._groupSettings._cellWidth / 2 - s.w / 2 + l,
                l += s.w + r.groupNodesSeparation
            }
        } else for (n = 0; n < t.bgnodes.length; n++) {
            var s; (s = t.bgnodes[n]).x = t._groupSettings._cellWidth / 2 - s.w / 2 + r.groupNodesSeparation + (t._groupSettings._cellWidth * i + r.groupNodesSeparation * i),
...
OrgChart.server._getResponse = function(t, e, r, i, a, o, n, l, s, h, d, c, g, p, f) {
    ...
    if (d == OrgChart.action.insert || d == OrgChart.action.expandCollapse || d == OrgChart.action.update || d == OrgChart.action.centerNode) {
        ...
        else if (d == OrgChart.action.expandCollapse && c && null != c.id && null != c.id)(k = g[c.id]) || (k = g[p[c.id]]),
        ...
        if (OrgChart.server._isVisible(I, i, s, d)) {
            ...
            if (d == OrgChart.action.expandCollapse && f && f[I.id] && "expandCollapseToLevel" == c.method) {
                ...
                T) {
                    ...
                    for (var M = I,
                    E = null; null != M;) f[M.id] && f[M.id].state == OrgChart.COLLAPSE && (E = M),
                    M = M.parent;
                    E && E.parent && (N = {
                        x: E.parent.x,
                        y: E.parent.y
                ...
                "all" == c.ids && T) {
                    ...
                    for (M = I, E = null; null != M;) f[M.id] && f[M.id].state == OrgChart.COLLAPSE && (E = M),
                    M = M.parent;
                    E && E.parent && (N = {
                        x: E.parent.x,
                        y: E.parent.y
...
OrgChart.server._setMinMaxXY = function(t, e) { (null == e.minX || null != t.x && t.x < e.minX) && (e.minX = t.x),
...
OrgChart.templates.base),
OrgChart.templates.group_orange.size = [250, 60],
...
OrgChart.templates.derek = Object.assign({},
OrgChart.templates.ana),
...
OrgChart.ui = {
    ...
    expandCollapse: function(t, e, r, i) {
        ...
        t.collapsedChildrenIds.length ? (a += OrgChart.expcollOpenTag.replace("{id}", t.id).replace("{x}", o).replace("{y}", n), a += l.plus) : (a += OrgChart.expcollOpenTag.replace("{id}", t.id).replace("{x}", o).replace("{y}", n), a += l.minus),
        a += OrgChart.grCloseTag
    },
    secondLinks: function(t, e, r, i, a) {
        if (null == e.spids) return "";
        for (var o = "",
        n = 0; n < e.spids.length; n++) {
            var l = [],
            s = e.spids[n],
            h = null,
            d = "left";
            if (t[s]) {
                ...
                g = r.levelSeparation; (e.parent && e.parent.layout == OrgChart.mixed || e.parent.layout == OrgChart.tree) && (g = r.mixedHierarchyNodesSeparation);
                var p = {
                ...
                if (e.level == t[s].level) switch (h = t[s], r.orientation) {
                case OrgChart.orientation.top:
                case OrgChart.orientation.top_left:
                    l.push([p.p, p.q]),
                    l.push([p.p, p.q - g / 3]),
                    ...
                    for (var f = e; null == h;) {
                        ...
                        if (C.id == s ? h = C: OrgChart._intersects(C, p, r) && (p = OrgChart._addPoint(C, l, r, p, d), u = !0), C.id != s) {
                            for (; m;) {
                                ...
                                OrgChart._intersects(m, p, r) && (p = OrgChart._addPoint(m, l, r, p, d), u = !0),
                                m = m.leftNeighbor
                            }
                            for (; O;) {
                                if (O.id == s) {
                                ...
                                OrgChart._intersects(O, p, r) && (p = OrgChart._addPoint(O, l, r, p, d), u = !0),
                                O = O.rightNeighbor
                            }
                        }
                        if (!u) {
                ...
                l = l.join(" "),
                o += OrgChart.secondLinkOpenTag.replace("{id}", e.id).replace("{child-id}", s) + c.secondLink.replace("{d}", l),
    ...
    link: function(t, e, r, i) {
        ...
        for (var l = 0; l < t.children.length; l++) {
            ...
            "" != c && (c = OrgChart.linkFieldsOpenTag.replace("{x}", h.x).replace("{y}", h.y).replace("{rotate}", 0) + c + OrgChart.grCloseTag, o.push(c)),
...
OrgChart._isHTML = function(t) {
    ...
    i = r.length; i--;) if (1 == r[i].nodeType) return ! 0;
    return ! 1
},
OrgChart.wrapText = function(t, e) {
    ...
    if (l = parseFloat(l), a || (a = 0), o || (o = 0), a || (n = "start"), s || (s = "ellipsis"), "ellipsis" == s) {
        ...
        d.textContent = t;
        for (var c = d.getComputedTextLength(), g = 2; l < c;) d.textContent = t.substring(0, t.length - g),
        d.textContent += "...",
    ...
    if ("multiline" == s) {
        ...
        for (var m = 1; m < p.length; m++) {
            ...
            if (u.firstChild.data += " " + p[m], u.getComputedTextLength() > l) u.firstChild.data = u.firstChild.data.slice(0, O),
        ...
        var v = "";
        if (null != d.innerHTML) v = d.innerHTML,
        d.innerHTML = "";
        else {
            var y = "";
            for (m = d.childNodes.length - 1; 0 <= m; m--) y = XMLSerializer().serializeToString(d.childNodes[m]) + y,
...
OrgChart.t = function(t, e) {
    ...
    if (null != e && r.scaleLessThen) {
        ...
        if (0 < a.length) {
            ...
            for (var s in l) null == i && (i = Object.assign({},
            r)),
            i[s] = l[s]
        }
    }
...
OrgChart._imgs2base64 = function(t, e, i, a) {
    ...
    function() {
        var e = l,
        r = o[e];
        OrgChart._getDataUri(r.getAttribute(i),
        function(t) {
...
OrgChart._json2csv = function(t) {
    for (var o = [], e = function(t) {
        for (var e = "",
        r = 0; r < o.length; r++) {
            var i; (i = "reportsTo" == o[r] ? null: null == t[o[r]] ? "": t[o[r]]) instanceof Date && (i = i.toLocaleString());
...
OrgChart.accentFold = function(t) {
    return (t = t.toString().toLowerCase()).replace(/([àáâãäå])|([ç])|([èéêë])|([ìíîï])|([ñ])|([òóôõöø])|([ß])|([ùúûü])|([ÿ])|([æ])/g,
...
OrgChart.prototype.zoom = function(t, e, r) {
    ...
    a = i,
    o = i[2],
    n = i[3]; ! 0 === t ? (i[2] = i[2] / OrgChart.SCALE_FACTOR, i[3] = i[3] / OrgChart.SCALE_FACTOR) : !1 === t ? (i[2] = i[2] * OrgChart.SCALE_FACTOR, i[3] = i[3] * OrgChart.SCALE_FACTOR) : (i[2] = i[2] / t, i[3] = i[3] / t),
    e || (e = [50, 50]),
    ...
    if (i[2] = this.width() / l, i[3] = this.height() / l, !0 === t && l < this.config.scaleMax || !1 === t && l > this.config.scaleMin || 0 != t && 1 != t && l < this.config.scaleMax && l > this.config.scaleMin) {
        ...
        this.config.anim.duration, this.config.anim.func,
        function() {
...
OrgChart.pdfPrevUI.show = function(r, i) {
    ...
    r.element.querySelector("#bg-prev-cancel").addEventListener("click",
    function() {
        OrgChart.pdfPrevUI.hide(r)
    }),
    r.element.querySelector("#bg-prev-save").addEventListener("click",
    function() {
        r.exportPDF(i),
        OrgChart.pdfPrevUI.hide(r)
    }),
...
OrgChart.pdfPrevUI._handler = function(t, e, r) {
    ...
    for (var s = l.querySelector("svg"), h = t.element.querySelector("#bg-ppdf-content"), d = 0; d < a.length; d++) {
        ...
        a[d].html ? g.write(OrgChart._exportHtml(a[d].html, i, a[d].innerSize.w, a[d].innerSize.h, p, f)) : (s.setAttribute("viewBox", a[d].vb), g.write(OrgChart._exportHtml(l.innerHTML, i, a[d].innerSize.w, a[d].innerSize.h, p, f)));
...
OrgChart._exportHtml = function(t, e, r, i, a, o) {
    for (var n = "",
    l = 0; l < e.margin.length; l++) n += e.margin[l] + "px ";
...
OrgChart.events.on("render",
function(t, e) {
    t._clink(t, e)
}),
OrgChart.prototype._clink = function(t, e) {
    for (var r, i, a, o, n, l = "",
    s = 0; s < this.config.clinks.length; s++) {
        ...
        if (d && c) {
            ...
            case OrgChart.orientation.top:
            case OrgChart.orientation.top_left:
                y <= x ? (_ = 1, O.x = y + d.w / 10, v.x = x - c.w / 10) : (_ = -1, O.x = y - d.w / 10, v.x = x + c.w / 10),
            ...
            case OrgChart.orientation.bottom_left:
                ...
                v.x = x + c.w / 10,
                p == u ? (_ = -1, O.y = p + d.h, v.y = u + c.h) : u < p ? (O.y = p, v.y = u + c.h) : (O.y = p + d.h, v.y = u);
                break;
            case OrgChart.orientation.left:
            case OrgChart.orientation.left_top:
                O.y = b <= w ? (_ = -1, b + d.h / 5) : (_ = 1, b - d.h / 5),
                v.y = w + c.h / 5,
                g == f ? (_ = -1, O.x = g, v.x = f) : f < g ? (O.x = g, v.x = f + c.w) : (O.x = g + d.w, v.x = f);
                break;
            case OrgChart.orientation.right:
            case OrgChart.orientation.right_top:
                O.y = b <= w ? (_ = 1, b + d.h / 5) : (_ = -1, b - d.h / 5),
                v.y = w + c.h / 5,
                g == f ? (_ = 1, O.x = g + d.w, v.x = f + c.w) : f < g ? (O.x = g, v.x = f + c.w) : (O.x = g + d.w, v.x = f)
            }
            var k = I(O, v, _),
            A = h.template;
            A || (A = "orange");
